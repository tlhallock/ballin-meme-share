...
First:
\begin{itemize}
\item What happens with two locals by the same name...
\item connections need to be revisited
\item get the mapper working again
\item implement find machines, find trackers, clear browsing data
\item make sure remote permissions are as up to date as possible
\item about thing
\item make sure files are added to local directory mirrors when their download is complete
\item change path of mirror?
\item make sure downloads are reinitiated
\item implement verify integrity
\item make sure things don't hang up when connections are manually closed
\item already downloaded frame
...
\item change tracker browser "Add known trackers" to "Add trackers"
\item debug already running
\item finish getLogs, downloadsbackup, messagesbackup
\item use json for messages (reencrypt the streams)
\item finish user manual
\item refresh downloads in gui more often
\item when cannot connect to machine ask tracker if the ip has changed
\item automatically sync synced trackers
\item close root synchronizer well when exception is thrown (and why is there a null pointer in RootSynchronizer.synchronize)
\end{itemize}
Later:
\begin{itemize}
...
\item implement popup for serves
